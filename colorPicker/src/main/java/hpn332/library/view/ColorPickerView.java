...
public class ColorPickerView extends View {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mShowAlphaPanel =
				a.getBoolean(R.styleable.colorpickerview__ColorPickerView_alphaChannelVisible,
				             false);
		mAlphaSliderText =
				a.getString(R.styleable.colorpickerview__ColorPickerView_alphaChannelText);
		mSliderTrackerColor =
				a.getColor(R.styleable.colorpickerview__ColorPickerView_sliderColor, 0xFFBDBDBD);
		mBorderColor =
				a.getColor(R.styleable.colorpickerview__ColorPickerView_borderColor, 0xFF6E6E6E);
		a.recycle();
		...
		mRequiredPadding =
				getResources().getDimensionPixelSize(R.dimen.colorpickerview__required_padding);
		...
		if (mHueBackgroundCache == null) {
			...
			for (int i = 0; i < hueColors.length; i++) {
				...
				mHueBackgroundCache.canvas
						.drawLine(0, i, mHueBackgroundCache.bitmap.getWidth(), i, linePaint);
			}
		}
	...
	private void setUpHueRect() {
		...
		int top    = dRect.top + BORDER_WIDTH_PX;
		int bottom = dRect.bottom - BORDER_WIDTH_PX -
				(mShowAlphaPanel ? (mPanelSpacingPx + mAlphaPanelHeightPx) : 0);
